...
class LegalIdentifyClauseTask:
    ...
    def grader_score(self):
        if not self.results: return {"final_score": 0.0}
        total = sum(r["score"] for r in self.results) / len(self.results)
        return {"task_id": self.TASK_ID, "final_score": total, "passed": total >= 0.7}
...
class LegalRiskFlagTask:
    ...
    def grader_score(self):
        ...
        return {"task_id": self.TASK_ID, "final_score": round(total, 4), "passed": total >= 0.6}
...
class LegalRedlineTask:
    ...
    def grader_score(self):
        total = sum(r["score"] for r in self.results) / len(self.results) if self.results else 0.0
        return {"task_id": self.TASK_ID, "final_score": total, "passed": total >= 0.6}
